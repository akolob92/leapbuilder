...
echo html_writer::link(new moodle_url('/mod/leapbuilder/editconcepttype.php', ['id' => $id]), 'Create concept type', ['class' => 'btn btn-success m-b-1']);

if (!$concepttypes = $DB->get_records_select('leapbuilder_concepttypes', 'leapbuilderid = ?', array($leapbuilder->id))) {
    echo '<br />You have no created concept types';
} else {
    ...
    $table->head[] = 'Commentary';
    $table->head[] = 'Created At';
    $table->head[] = 'Actions';
//    $table->align = array('left', 'left', 'left');

    foreach ($concepttypes as $value) {
        $editlink = html_writer::link(new moodle_url('/mod/leapbuilder/editconcepttype.php', ['id' => $id, 'concepttypeid' => $value->id]), 'Edit', ['class' => 'btn btn-warning m-r-1']);
        $dellink = html_writer::link(new moodle_url('/mod/leapbuilder/editconcepttype.php', ['id' => $id, 'concepttypeid' => $value->id, 'remove' => true]), 'Delete', ['class' => 'btn btn-danger']);
        $cells = array(
            new html_table_cell($value->id),
            new html_table_cell($value->name),
            new html_table_cell($value->importance),
            new html_table_cell($value->comment),

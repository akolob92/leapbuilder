...
echo html_writer::link(new moodle_url('/mod/leapbuilder/editrelation.php', ['id' => $id]), 'Create relation', ['class' => 'btn btn-success m-b-1']);

if (!$relations = $DB->get_records_select('leapbuilder_relations', 'leapbuilderid = ?', array($leapbuilder->id))) {
    echo '<br />You have no created relations!';
} else {
    ...
    $table->head = array();
    $table->head[] = 'ID';
    $table->head[] = 'Source Concept';
    $table->head[] = 'Target Concept';
    $table->head[] = 'Influence';
    $table->head[] = 'Created At';
    $table->head[] = 'Actions';
//    $table->align = array('left', 'left', 'left');

    foreach ($relations as $value) {
        $editlink = html_writer::link(new moodle_url('/mod/leapbuilder/editrelation.php', ['id' => $id, 'relationid' => $value->id]), 'Edit', ['class' => 'btn btn-warning m-r-1']);
        $dellink = html_writer::link(new moodle_url('/mod/leapbuilder/editrelation.php', ['id' => $id, 'relationid' => $value->id, 'remove' => true]), 'Delete', ['class' => 'btn btn-danger']);
        $cells = array(
            new html_table_cell($value->id),
            new html_table_cell(html_writer::link(new moodle_url('/mod/leapbuilder/editconcept.php', ['id' => $id, 'conceptid' => $value->fromconceptid]), $value->fromconceptid)),
            new html_table_cell(html_writer::link(new moodle_url('/mod/leapbuilder/editconcept.php', ['id' => $id, 'conceptid' => $value->toconceptid]), $value->toconceptid)),
            new html_table_cell($value->influence),
